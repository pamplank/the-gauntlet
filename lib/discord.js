...
// Mirrors the ranking on /weeks/[weekId] exactly: wounds ascending, then most
// games played. If the two ever diverge the Discord post becomes a liar, so
// this is worth keeping in step by hand.
export async function announceWeekResults(weekId) {
  ...
  const ranked = Object.values(stats)
    .filter((p) => p.played > 0)
    .sort((a, b) => a.wounds - b.wounds || b.played - a.played);
  ...
  const podium = ranked
    .slice(0, 3)
    .map((p, i) => `${medals[i]} **${p.name}** — ${p.wounds} wounds over ${p.played} games`)
    .join("\n");
